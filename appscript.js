...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "update_affiliate_commission_payment": return jsonRes(updateAffiliateCommissionPayment(data, cfg));
...
/* =========================
   PAGINATION ACTIONS
========================= */
function normalizeCommissionPaymentStatus_(status) {
  const raw = String(status || "").trim().toUpperCase();
  if (raw === "SUDAH_DIBAYAR" || raw === "PAID" || raw === "LUNAS") return "SUDAH_DIBAYAR";
  if (raw === "SEDANG_DIPROSES" || raw === "PROCESSING" || raw === "IN_PROGRESS") return "SEDANG_DIPROSES";
  return "BELUM_DIBAYAR";
}

function isValidCommissionStatusTransition_(fromStatus, toStatus) {
  const from = normalizeCommissionPaymentStatus_(fromStatus);
  const to = normalizeCommissionPaymentStatus_(toStatus);
  if (from === "SUDAH_DIBAYAR") return to === "SUDAH_DIBAYAR";
  if (from === "SEDANG_DIPROSES") return to === "SEDANG_DIPROSES" || to === "BELUM_DIBAYAR" || to === "SUDAH_DIBAYAR";
  return to === "BELUM_DIBAYAR" || to === "SEDANG_DIPROSES" || to === "SUDAH_DIBAYAR";
}
...
function getCommissionPaymentSheet_() {
  const name = "Affiliate_Commission_Payments";
  let s = ss.getSheetByName(name);
  const headers = [
    "invoice", "affiliate_id", "commission_nominal", "payout_status",
    "paid_at", "paid_amount", "payment_method", "transfer_ref",
  ...
  const existingHeaders = s.getRange(1, 1, 1, width).getValues()[0];
  let requiresHeaderFix = false;
  for (let i = 0; i < width; i++) {
    if (String(existingHeaders[i] || "").trim() !== headers[i]) {
      requiresHeaderFix = true;
      break;
    }
  }
  if (requiresHeaderFix) {
    s.getRange(1, 1, 1, width).setValues([headers]);
  }
  return s;
}

function getCommissionPaymentAuditSheet_() {
  const name = "Affiliate_Commission_Payment_Audit";
  let s = ss.getSheetByName(name);
  const headers = [
    "timestamp", "invoice", "affiliate_id", "from_status", "to_status",
    "commission_nominal", "paid_amount", "payment_method", "transfer_ref",
    "admin_email", "admin_name", "request_id", "note"
  ];
  if (!s) {
    s = ss.insertSheet(name);
    s.appendRow(headers);
...
function appendCommissionPaymentAudit_(entry) {
  const audit = getCommissionPaymentAuditSheet_();
  audit.appendRow([
    new Date().toISOString(),
    entry.invoice || "",
    entry.affiliate_id || "",
    normalizeCommissionPaymentStatus_(entry.from_status),
    normalizeCommissionPaymentStatus_(entry.to_status),
    toNumberSafe_(entry.commission_nominal),
    toNumberSafe_(entry.paid_amount),
...
function findOrderCommissionContext_(invoice) {
  const ordersSheet = mustSheet_("Orders");
  const rows = ordersSheet.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    if (String(row[0]) === String(invoice)) {
      const affiliateId = String(row[9] || "-").trim() || "-";
      const commissionNominal = toNumberSafe_(row[10]);
      return {
        found: true,
        row_index: i + 1,
        invoice: String(row[0] || ""),
        affiliate_id: affiliateId,
        commission_nominal: commissionNominal,
        is_commission_eligible: affiliateId !== "-" && commissionNominal > 0,
        order_status: String(row[7] || "").trim(),
        row: row
      };
    }
  }
...
function buildCommissionPaymentInfo_(orderRow, record) {
  const inv = String(orderRow[0] || "");
  const affiliateId = String(orderRow[9] || "-").trim() || "-";
  const commissionNominal = toNumberSafe_(orderRow[10]);
  const orderStatus = String(orderRow[7] || "").trim();
  const eligible = affiliateId !== "-" && commissionNominal > 0;
  ...
  if (!eligible) {
    return {
      ...
      is_paid: false,
      can_update: false,
      lock_reason: "Order tidak memiliki komisi affiliate"
    };
  }

  const status = normalizeCommissionPaymentStatus_(record ? record[3] : "BELUM_DIBAYAR");
  const isPaid = status === "SUDAH_DIBAYAR";
  const orderLunas = String(orderStatus).toLowerCase() === "lunas";
  let lockReason = "";

  if (!orderLunas) lockReason = "Order belum lunas";
  if (isPaid) lockReason = "Komisi sudah lunas";
  ...
  return {
    ...
    paid_at: record ? String(record[4] || "") : "",
    paid_amount: record ? toNumberSafe_(record[5]) : 0,
    payment_method: record ? String(record[6] || "") : "",
    transfer_ref: record ? String(record[7] || "") : "",
    confirmed_by: record ? String(record[8] || "") : "",
    confirmed_at: record ? String(record[9] || "") : "",
    updated_at: record ? String(record[10] || "") : "",
    is_eligible: true,
    is_paid: isPaid,
    can_update: orderLunas && !isPaid,
    lock_reason: lockReason
  };
}

function updateAffiliateCommissionPayment(d, cfg) {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) {
    return { status: "error", message: "Sistem sedang memproses konfirmasi lain. Coba lagi beberapa detik." };
  }

  try {
    const session = requireAdminSession_(d, { allowDemo: false, actionName: "update_affiliate_commission_payment" });
    cfg = cfg || getSettingsMap_();

    const invoice = String(d.id || d.invoice || "").trim();
    if (!invoice) return { status: "error", message: "Invoice wajib diisi" };

    const ctx = findOrderCommissionContext_(invoice);
    if (!ctx.found) return { status: "error", message: "Order tidak ditemukan" };
    if (!ctx.is_commission_eligible) {
      return { status: "error", message: "Order ini tidak memiliki komisi affiliate yang bisa dibayarkan" };
    }

    const targetStatus = normalizeCommissionPaymentStatus_(d.payout_status || d.status);
    const orderLunas = String(ctx.order_status || "").toLowerCase() === "lunas";
    if (!orderLunas && targetStatus !== "BELUM_DIBAYAR") {
      return { status: "error", message: "Komisi hanya bisa diproses setelah order berstatus Lunas" };
    }

    const paymentSheet = getCommissionPaymentSheet_();
    const rows = paymentSheet.getDataRange().getValues();
    let rowIndex = -1;
    let currentRecord = null;
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === ctx.invoice && String(rows[i][1]) === ctx.affiliate_id) {
        rowIndex = i + 1;
    ...
    if (!currentRecord) {
      ...
      rowIndex = paymentSheet.getLastRow();
      currentRecord = paymentSheet.getRange(rowIndex, 1, 1, 11).getValues()[0];
    }

    const currentStatus = normalizeCommissionPaymentStatus_(currentRecord[3]);
    if (currentStatus === "SUDAH_DIBAYAR" && targetStatus === "SUDAH_DIBAYAR") {
      return {
        status: "success",
        message: "Komisi sudah ditandai lunas sebelumnya. Konfirmasi ganda otomatis ditolak.",
        duplicate_prevented: true,
        commission_payment: buildCommissionPaymentInfo_(ctx.row, currentRecord)
      };
    }
    if (!isValidCommissionStatusTransition_(currentStatus, targetStatus)) {
      return { status: "error", message: "Status komisi yang sudah lunas tidak boleh diubah lagi" };
    }

    const paidAmount = toNumberSafe_(d.paid_amount);
    const paymentMethod = String(d.payment_method || "").trim();
    const transferRef = String(d.transfer_ref || "").trim();
    const paidAt = normalizeDateOnly_(d.paid_at || "");
    const adminEmail = String(session.email || "").trim();
    const adminName = String(session.name || "").trim();
    const adminLabel = adminName ? (adminName + (adminEmail ? " (" + adminEmail + ")" : "")) : (adminEmail || String(session.id || "admin"));
    const requestId = String(d.request_id || ("COMM-" + Date.now())).trim();
    const note = String(d.note || "").trim();

    if (targetStatus === "SUDAH_DIBAYAR") {
      if (!paidAt) return { status: "error", message: "Tanggal pembayaran wajib diisi" };
      if (paidAmount <= 0) return { status: "error", message: "Nominal dibayarkan wajib lebih dari 0" };
      if (paidAmount > ctx.commission_nominal) {
        return { status: "error", message: "Nominal dibayarkan tidak boleh melebihi komisi order" };
      }
      if (!paymentMethod) return { status: "error", message: "Metode pembayaran wajib diisi" };
      if (!transferRef) return { status: "error", message: "Referensi transfer wajib diisi" };
    }
    ...
    if (targetStatus === "SUDAH_DIBAYAR") {
      ...
      nextRef = transferRef;
    } else if (targetStatus === "SEDANG_DIPROSES") {
      nextMethod = paymentMethod;
      nextRef = transferRef;
    }
    ...
    paymentSheet.getRange(rowIndex, 1, 1, 11).setValues([nextRecord]);

    appendCommissionPaymentAudit_({
      invoice: ctx.invoice,
      affiliate_id: ctx.affiliate_id,
      from_status: currentStatus,
      to_status: targetStatus,
      commission_nominal: ctx.commission_nominal,
      paid_amount: nextPaidAmount || 0,
      payment_method: nextMethod,
      transfer_ref: nextRef,
    ...
    return withPublicCacheState_({
      status: "success",
      message: "Status pembayaran komisi berhasil diperbarui",
      duplicate_prevented: false,
      commission_payment: buildCommissionPaymentInfo_(ctx.row, nextRecord)
    }, cacheState);
  } catch (e) {
    return { status: "error", message: e.toString() };
  } finally {
...
function getAdminOrders(d) {
  try {
    ...
    const paymentMap = {};
    let paymentRows = [];
    const paymentSheet = ss.getSheetByName("Affiliate_Commission_Payments");
    if (paymentSheet && paymentSheet.getLastRow() > 1) {
      paymentRows = paymentSheet.getDataRange().getValues();
    }
    ...
    for (let i = 1; i < paymentRows.length; i++) {
      ...
      if (key !== "::") paymentLookup[key] = row;
    }

    for (let j = 0; j < pageRows.length; j++) {
      const row = pageRows[j];
      const key = String(row[0] || "") + "::" + String(row[9] || "-");
      paymentMap[String(row[0] || "")] = buildCommissionPaymentInfo_(row, paymentLookup[key] || null);
    }
    
    return {
      status: "success",
      data: pageRows,
      commission_payment_map: paymentMap,
